...
from nacsos_data.db.schemas import BotAnnotationMetaData, AssignmentScope, User, Annotation, BotAnnotation, Assignment
from nacsos_data.models.annotations import AnnotationSchemeModel, AssignmentScopeModel, AssignmentModel, AssignmentStatus, AnnotationSchemeModelFlat
...
from nacsos_data.util.annotations.resolve import get_resolved_item_annotations, read_annotation_scheme
from nacsos_data.util.annotations.validation import merge_scheme_and_annotations, annotated_scheme_to_annotations, flatten_annotation_scheme
...
@router.get('/schemes/definition/{annotation_scheme_id}', response_model=AnnotationSchemeModelFlat | AnnotationSchemeModel)
async def get_scheme_definition(
    ...
    """
    scheme: AnnotationSchemeModel | None = await read_annotation_scheme(annotation_scheme_id=annotation_scheme_id, db_engine=db_engine)
...
async def get_annotation_scheme_fingerprints(
    ...
    async with db_engine.session() as session:  # type: AsyncSession
        ...
        return {row['annotation_scheme_id']: row['hash'] for row in rslt}
...
async def _construct_annotation_item(assignment: AssignmentModel, project_id: str | uuid.UUID) -> AnnotationItem:
    ...
    if scope is None:
        raise AnnotationSchemeNotFoundError(f'No annotation scope found in DB for id {assignment.assignment_scope_id}')

    scheme = await read_annotation_scheme(annotation_scheme_id=assignment.annotation_scheme_id, db_engine=db_engine)
    if scheme is None:
        raise AnnotationSchemeNotFoundError(f'No annotation scheme found in DB for id {assignment.annotation_scheme_id}')
...
async def get_next_assignment_for_scope_for_user(
    ...
    assignment = await read_next_assignment_for_scope_for_user(
        current_assignment_id=current_assignment_id, assignment_scope_id=assignment_scope_id, user_id=permissions.user.user_id, db_engine=db_engine
    )
...
async def get_next_open_assignment_for_scope_for_user(
    ...
    assignment = await read_next_open_assignment_for_scope_for_user(
        assignment_scope_id=assignment_scope_id, user_id=permissions.user.user_id, db_engine=db_engine
    )
    # Either there are no assignments, or everything is done.
    if assignment is None:
        assignments = await read_assignments_for_scope_for_user(
            assignment_scope_id=assignment_scope_id, user_id=permissions.user.user_id, db_engine=db_engine, limit=1
        )
...
async def get_assignment(
    ...
    if (assignment is None) or (assignment.user_id != permissions.user.user_id):
        raise HTTPException(status_code=http_status.HTTP_401_UNAUTHORIZED, detail='You do not have permission to handle this assignment, as it is not yours!')
...
async def get_assignment_scopes_for_user(
    ...
    scopes = await read_assignment_scopes_for_project_for_user(project_id=project_id, user_id=permissions.user.user_id, db_engine=db_engine)
    return scopes
...
async def get_assignment_scope(
    assignment_scope_id: str,
    permissions: UserPermissions = Depends(UserPermissionChecker(['annotations_read', 'annotations_edit'], fulfill_all=False)),
...
async def get_assignments(
    ...
    assignments = await read_assignments_for_scope_for_user(assignment_scope_id=assignment_scope_id, user_id=permissions.user.user_id, db_engine=db_engine)
    return assignments
...
async def get_assignment_indicators_for_scope(
    ...
    return await read_assignment_overview_for_scope(assignment_scope_id=assignment_scope_id, connection=db_engine)
...
async def get_assignments_for_scope(
    ...
    assignments = await read_assignments_for_scope(assignment_scope_id=assignment_scope_id, db_engine=db_engine)
    return assignments
...
async def save_annotation(
    ...
    if (
        permissions.user.user_id == assignment_db.user_id
        ...
        status = await upsert_annotations(annotations=annotations, assignment_id=annotated_item.assignment.assignment_id, db_engine=db_engine)
...
async def get_items_with_count(
    ...
    items = await read_item_ids_with_assignment_count_for_project(project_id=permissions.permissions.project_id, db_engine=db_engine)
    return items
...
async def make_assignments(
    ...
    async with db_engine.session() as session:  # type: AsyncSession
        await create_assignments(session=session, assignment_scope_id=assignment_scope_id, project_id=permissions.permissions.project_id)
...
async def bulk_add_assignment(
    ...
    async with db_engine.session() as session:  # type: AsyncSession
        ...
        existing_ids = {
            str(eid)
            for eid in (
                await session.execute(select(Assignment.item_id).where(Assignment.assignment_scope_id == info.scope_id, Assignment.user_id == info.user_id))
            ).scalars()
        }
    ...
    async with db_engine.session() as session:  # type: AsyncSession
        ...
        n_annotations: int = (
            await session.execute(  # type: ignore[assignment]
                ...
                .join(Assignment)
                .where(Assignment.item_id == info.item_id, Assignment.user_id == info.user_id, Assignment.assignment_scope_id == info.scope_id)
            )
        ).scalar()
...
async def get_assignment_scopes_for_scheme(
    ...
    async with db_engine.session() as session:  # type: AsyncSession
        scopes = await session.execute(select(AssignmentScope).where(AssignmentScope.annotation_scheme_id == scheme_id))
...
async def get_annotators_for_scheme(
    ...
    async with db_engine.session() as session:  # type: AsyncSession
        return [
            UserModel.model_validate(user.__dict__)
            for user in (await session.execute(select(User).join(Annotation).distinct().where(Annotation.annotation_scheme_id == scheme_id))).scalars().all()
        ]
...
async def get_resolved_annotations(
    ...
    if bot_annotation_metadat_id is not None:
        return await read_resolved_bot_annotations(
            db_engine=db_engine,
    ...
    return await get_resolved_item_annotations(
        strategy=settings.algorithm,
...
async def get_saved_resolved_annotations(
    ...
    async with db_engine.session() as session:  # type: AsyncSession
        bot_meta = await read_resolved_bot_annotation_meta(bot_annotation_metadata_id=bot_annotation_metadata_id, session=session)
        proposal = await read_resolved_bot_annotations_for_meta(
            session=session, bot_meta=bot_meta, include_new=False, include_empty=False, update_existing=False
        )
...
async def update_resolved_annotations(
    ...
    await update_resolved_bot_annotations(bot_annotation_metadata_id=bot_annotation_metadata_id, name=name, matrix=matrix, db_engine=db_engine, use_commit=True)
...
async def list_saved_resolved_annotations(
    ...
    async with db_engine.session() as session:  # type: AsyncSession
        stmt = (
            ...
            .where(BotAnnotationMetaData.project_id == permissions.permissions.project_id, BotAnnotationMetaData.kind == BotKind.RESOLVE)
            .order_by(BotAnnotationMetaData.time_created)
            .options(
                load_only(
                    BotAnnotationMetaData.bot_annotation_metadata_id,
...
async def delete_saved_resolved_annotations(
    ...
    async with db_engine.session() as session:  # type: AsyncSession
        meta: BotAnnotationMetaData | None = (
...
async def get_bot_annotations(
    ...
    async with db_engine.session() as session:  # type: AsyncSession
        stmt = (
            ...
            .join(BotAnnotation, BotAnnotation.bot_annotation_metadata_id == BotAnnotationMetaData.bot_annotation_metadata_id)
        ...
        if rslt:
            return [
                BotMetaInfo.model_validate(
                    {**r['BotAnnotationMetaData'].__dict__, 'num_annotations': r['num_annotations'], 'num_annotated_items': r['num_annotated_items']}
                )
                for r in rslt
            ]
        return []
...
async def get_bot_scopes(
    ...
    async with db_engine.session() as session:  # type: AsyncSession
        ...
        return [BotAnnotationMetaDataModel(**r.__dict__) for r in (await session.execute(stmt)).scalars().all()]

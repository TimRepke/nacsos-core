...
from nacsos_data.db.schemas import AnnotationTracker, AssignmentScope, AnnotationScheme, BotAnnotationMetaData, AnnotationQuality
...
async def get_project_scopes(permissions: UserPermissions = Depends(UserPermissionChecker('annotations_read'))) -> list[LabelScope]:
    async with db_engine.session() as session:  # type: AsyncSession
        stmt = (
            select(AssignmentScope.assignment_scope_id.cast(String).label('scope_id'), AssignmentScope.name, literal('H', type_=String).label('scope_type'))
        ...
        stmt = (
            select(
                BotAnnotationMetaData.bot_annotation_metadata_id.cast(String).label('scope_id'),
...
async def get_resolutions_for_scope(
    assignment_scope_id: str, permissions: UserPermissions = Depends(UserPermissionChecker('annotations_read'))
) -> list[BotAnnotationMetaDataBaseModel]:
    async with db_engine.session() as session:  # type: AsyncSession
        stmt = select(BotAnnotationMetaData).where(BotAnnotationMetaData.assignment_scope_id == assignment_scope_id)
        rslt = (await session.execute(stmt)).scalars().all()
        return [BotAnnotationMetaDataBaseModel.model_validate(r.__dict__) for r in rslt]


async def read_tracker(session: AsyncSession, tracker_id: str | uuid.UUID, project_id: str | uuid.UUID | None = None) -> AnnotationTracker:
    stmt = select(AnnotationTracker).where(AnnotationTracker.annotation_tracking_id == tracker_id)
...
async def get_project_trackers(permissions: UserPermissions = Depends(UserPermissionChecker('annotations_read'))) -> list[DehydratedAnnotationTracker]:
    async with db_engine.session() as session:  # type: AsyncSession
        stmt = select(AnnotationTracker.name, AnnotationTracker.annotation_tracking_id).where(
            AnnotationTracker.project_id == permissions.permissions.project_id
        )
        rslt = (await session.execute(stmt)).mappings().all()
...
async def get_tracker(tracker_id: str, permissions: UserPermissions = Depends(UserPermissionChecker('annotations_read'))) -> AnnotationTrackerModel:
    async with db_engine.session() as session:  # type: AsyncSession
        tracker = await read_tracker(tracker_id=tracker_id, session=session, project_id=permissions.permissions.project_id)
        return AnnotationTrackerModel.model_validate(tracker.__dict__)


@router.put('/tracking/tracker', response_model=str)
async def save_tracker(tracker: AnnotationTrackerModel, permissions: UserPermissions = Depends(UserPermissionChecker('annotations_read'))) -> str:
    pkey = await upsert_orm(
        upsert_model=tracker,
        Schema=AnnotationTracker,
...
async def update_tracker(
    ...
    async with db_engine.session() as session:  # type: AsyncSession
        ...
        batched_annotations = [await get_annotations(session=session, source_ids=[sid]) for sid in tracker.source_ids]

        batched_sequence = [
            annotations_to_sequence(tracker.inclusion_rule, annotations=annotations, majority=tracker.majority)
...
async def get_irr(
    ...
    async with db_engine.session() as session:  # type: AsyncSession
        results = (await session.execute(select(AnnotationQuality).where(AnnotationQuality.assignment_scope_id == assignment_scope_id))).scalars().all()
...
async def recompute_irr(
    ...
    async with db_engine.session() as session:  # type: AsyncSession
        ...
        await session.execute(delete(AnnotationQuality).where(AnnotationQuality.assignment_scope_id == assignment_scope_id))
        # Compute new metrics
        metrics = await compute_irr_scores(
            session=session, assignment_scope_id=assignment_scope_id, resolution_id=bot_annotation_metadata_id, project_id=permissions.permissions.project_id
        )

...
async def get_project_highlighters(permissions: UserPermissions = Depends(UserPermissionChecker('annotations_read'))) -> list[HighlighterModel]:
...
async def upsert_highlighter(highlighter: HighlighterModel, permissions: UserPermissions = Depends(UserPermissionChecker('annotations_edit'))) -> str:
    ...
    async with db_engine.session() as session:  # type: AsyncSession
        stmt = select(Highlighter).where(Highlighter.project_id == permissions.permissions.project_id, Highlighter.highlighter_id == highlighter.highlighter_id)
...
async def get_highlighter(highlighter_id: str, permissions: UserPermissions = Depends(UserPermissionChecker('annotations_read'))) -> HighlighterModel:
    async with db_engine.session() as session:  # type: AsyncSession
        stmt = select(Highlighter).where(Highlighter.project_id == permissions.permissions.project_id, Highlighter.highlighter_id == highlighter_id)
        result = (await session.scalars(stmt)).one_or_none()
        if result is not None:
            return HighlighterModel.model_validate(result.__dict__)
        raise NoDataForKeyError(f'No highlighter in project {permissions.permissions.project_id} with id {highlighter_id}!')

...
from nacsos_data.db.crud.imports import read_import, upsert_import, delete_import, read_item_count_for_import
...
async def get_project_imports(
    ...
    async with db_engine.session() as session:  # type: AsyncSession
        ...
        return [
            ImportDetails(
                **ii['Import'].__dict__, revisions=([] if not ii['revisions'] else [ImportRevisionModel(**rev) for rev in ii['revisions'] if rev is not None])
            )
            for ii in rslt
        ]
...
async def get_import_details(import_id: str, permissions: UserPermissions = Depends(UserPermissionChecker('imports_read'))) -> ImportModel:
...
async def get_import_revisions(
    ...
    async with db_engine.session() as session:  # type: AsyncSession
        ...
        # FIXME: add permission check for project_id
        return [
            ImportRevisionDetails(**ird['ImportRevision'].__dict__, task=None if not ird['Task'] else ird['Task'].__dict__) for ird in rslt.mappings().all()
        ]
...
async def trigger_import(
    ...
    import_details = await read_import(import_id=import_id, engine=db_engine)
    if import_details is not None and str(import_details.project_id) == str(permissions.permissions.project_id):
        tasks.imports.import_task.send(
            project_id=str(import_details.project_id),  # type: ignore[call-arg]

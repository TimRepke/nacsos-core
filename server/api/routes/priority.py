...
@router.post('/table/peek/html')
async def get_table_sample_html(params: PrioTableParams,
                                permissions: UserPermissions = Depends(UserPermissionChecker('dataset_read'))) -> str:
    df = await _get_df(project_id=permissions.permissions.project_id,
...
@router.post('/table/peek')
async def get_table_sample(params: PrioTableParams,
                           permissions: UserPermissions = Depends(UserPermissionChecker('dataset_read'))) -> list[
    dict[str, Any]]:
    df = await _get_df(project_id=permissions.permissions.project_id,
                       scope_ids=params.scope_ids,
                       incl=params.incl,
...
async def read_project_setup(permissions: UserPermissions = Depends(UserPermissionChecker('annotations_prio'))) \
...
async def read_prio_setup(priority_id: str,
                          permissions: UserPermissions = Depends(UserPermissionChecker('annotations_prio')))->PriorityModel|None:
    async with db_engine.session() as session:  # type: AsyncSession

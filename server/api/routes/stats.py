...
async def get_basic_stats(
    ...
    async with db_engine.session() as session:  # type: AsyncSession
        num_items: int = await session.scalar(sa.select(sa.func.count(Item.item_id))
                                              .where(Item.project_id == project_id)) or 0
        num_imports: int = await session.scalar(sa.select(sa.func.count(Import.import_id))
                                                .where(Import.project_id == project_id)) or 0
        num_schemes: int = await session.scalar(sa.select(sa.func.count(AnnotationScheme.annotation_scheme_id))
                                                .where(AnnotationScheme.project_id == project_id)) or 0
        num_scopes: int = await session.scalar(sa.select(sa.func.count(AssignmentScope.assignment_scope_id))
                                               .join(AnnotationScheme,
                                                     AnnotationScheme.annotation_scheme_id == AssignmentScope.annotation_scheme_id)
                                               .where(AnnotationScheme.project_id == project_id)) or 0
        num_labels: int = await session.scalar(sa.select(sa.func.count(Annotation.annotation_id))
                                               .join(AnnotationScheme,
                                                     AnnotationScheme.annotation_scheme_id == Annotation.annotation_scheme_id)
                                               .where(AnnotationScheme.project_id == project_id)) or 0
        num_labeled_items: int = await session.scalar(sa.select(sa.func.count(sa.func.distinct(Annotation.item_id)))
                                                      .join(AnnotationScheme,
                                                            AnnotationScheme.annotation_scheme_id == Annotation.annotation_scheme_id)
                                                      .where(AnnotationScheme.project_id == project_id)) or 0
...
async def label_stats(
        query: NQLFilter | None = Body(default=None),

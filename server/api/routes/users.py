...
from nacsos_data.db.crud.users import read_users, read_user_by_id, read_users_by_ids, create_or_update_user, get_password_hash
...
async def get_all_users(current_user: UserModel = Depends(get_current_active_user)) -> list[UserInDBModel]:
...
async def get_all_users_dehydrated(current_user: UserModel = Depends(get_current_active_user)) -> list[UserInDBModel]:
    result = await read_users(project_id=None, order_by_username=True, engine=db_engine)
    if result is None:
        return []
    return result
...
async def get_project_annotator_users(project_id: str, permissions: UserPermissions = Depends(UserPermissionChecker())) -> dict[str, UserBaseModel]:
    ...
    async with db_engine.session() as session:
        ...
        return {str(res.user_id): UserBaseModel(**res.__dict__) for res in result}


@router.get('/list/project/{project_id}', response_model=list[UserBaseModel])
async def get_project_users(project_id: str, permissions: UserPermissions = Depends(UserPermissionChecker())) -> list[UserInDBModel]:
...
async def get_user_by_id(user_id: str, permissions: UserPermissions = Depends(UserPermissionChecker('annotations_edit'))) -> UserInDBModel:
...
async def get_users_by_ids(
    user_id: list[str] = Query(), permissions: UserPermissions = Depends(UserPermissionChecker('annotations_edit'))
) -> list[UserInDBModel]:
...
async def save_user_self(
    ...
    async with db_engine.session() as session:  # type: AsyncSession
        user_db: User | None = (await session.scalars(select(User).where(User.user_id == str(current_user.user_id)))).one_or_none()
